...
def write_postgis(
    ...
    if isinstance(df, GeoDataFrame):
        df.to_postgis(
            table, create_engine("postgresql+psycopg://", creator=lambda: connection), schema=temporary_schema, if_exists="replace", index=False
        )
    else:
        df.to_sql(
            table, create_engine("postgresql+psycopg://", creator=lambda: connection), schema=temporary_schema, if_exists="replace", index=False
    ...
    logger.debug("Data has columns: %s", df.columns)
    id_column = [column for column in ("objektidentitet",) if column in df.columns][0]
    ...
    """).format(
                        ...
                        sql.Composed([sql.Identifier(i) for i in df.columns]).join(", "),
                        sql.Composed(
                            [
